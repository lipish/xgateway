...
interface ServiceCreateDialogProps {
  ...
  form: {
    ...
    bound_provider_ids: number[]
    qps_limit: number
    concurrency_limit: number
    max_queue_size: number
    max_queue_wait_ms: number
  }
  onFormChange: (next: ServiceCreateDialogProps["form"]) => void
  onSave: () => void
  saving: boolean
  error: string | null
}
...
export function ServiceCreateDialog({ open, onOpenChange, services, providers, form, onFormChange, onSave, saving, error }: ServiceCreateDialogProps) {
  ...
  const selectedProviderIdSet = new Set(form.bound_provider_ids)
  const selectedProviderNames = providers
    .filter((p) => selectedProviderIdSet.has(p.id))
    .map((p) => p.name)
                      ...
                      <CommandGroup>
                        {providers.map((p) => (
                          <CommandItem
                            ...
                            onSelect={() => {
                              const exists = selectedProviderIdSet.has(p.id)
                              const next = exists ? form.bound_provider_ids.filter((id) => id !== p.id) : [...form.bound_provider_ids, p.id]
                              onFormChange({ ...form, bound_provider_ids: next })
                            }}
                          >
                            <Check className={cn("mr-2 h-4 w-4", selectedProviderIdSet.has(p.id) ? "opacity-100" : "opacity-0")} />
