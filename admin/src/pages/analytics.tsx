            ...
            <div className="rounded-xl border bg-card p-6 h-80 flex flex-col min-h-0">
              ...
              <div className="space-y-2">
                {data.top_models.length > 0 ? (
                  data.top_models.slice(0, 4).map((model, index) => (
                    <div key={index} className="flex items-center justify-between px-3 py-2 bg-muted/50 rounded-lg gap-3 min-w-0">
                      <div className="min-w-0">
                        <div className="font-medium text-sm truncate">{model.model}</div>
                        <div className="text-xs text-muted-foreground">{model.requests} requests</div>
                      </div>
                      <div className="text-right shrink-0">
                        <div className="font-medium text-sm">{model.tokens.toLocaleString()}</div>
