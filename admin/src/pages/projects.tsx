...
export function ProjectsPage() {
  ...
  const orgOptions = orgs.map((o) => ({ value: String(o.id), label: `${o.name} (#${o.id})` }))
  ...
  return (
    <div className="flex-1 min-h-0 h-full flex flex-col page-transition p-6 scrollbar-hide">
      <PageHeader
        ...
        onRefresh={() => fetchProjects(filterOrgId)}
        loading={loading}
      />
          ...
          left={
            <Card className="w-[420px] max-w-full min-h-0 overflow-hidden">
              <CardContent className="p-4 space-y-4">
                ...
                <div className="space-y-2">
                  <div className="text-sm font-medium">{t("projects.create")}</div>
                  <Select
                  ...
                  <div className="flex gap-2">
                    <Input value={createName} onChange={(e) => setCreateName(e.target.value)} placeholder={t("projects.namePlaceholder")} className="h-10" />
                    <Button onClick={handleCreate} disabled={saving || !createName.trim() || !createOrgId} className="h-10 px-4">
                      {t("common.save")}
                    </Button>
                  </div>
                </div>
                  ...
                  {loading ? (
                    ...
                    <div className="space-y-1">
                      {projects.map((p) => {
                        ...
                        const orgName = orgs.find((o) => o.id === p.org_id)?.name
                        return (
                          <button
                            key={p.id}
                            type="button"
                          ...
                          >
                            ...
                            <div className="text-xs text-muted-foreground truncate">{orgName ? `${orgName} (#${p.org_id})` : `org #${p.org_id}`}</div>
                          </button>
                        )
                      })}
                    </div>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <Card>
                      <CardContent className="p-4 space-y-1">
                        ...
                        <div className="text-sm font-medium">{selectedProject.updated_at}</div>
                      </CardContent>
                    </Card>
                  </div>
