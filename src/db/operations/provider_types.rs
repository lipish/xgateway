...
impl DatabasePool {
    ...
    pub async fn create_provider_type(&self, pt: NewProviderType) -> Result<()> {
        ...
        match self {
            Self::Sqlite(pool) => {
                sqlx::query(
                    "INSERT INTO provider_types (id, label, base_url, models, driver_type, enabled, sort_order, docs_url) VALUES (?, ?, ?, ?, ?, ?, ?, ?)"
                )
                .bind(&pt.id).bind(&pt.label).bind(&pt.base_url)
            ...
            Self::Postgres(pool) => {
                sqlx::query(
                    "INSERT INTO provider_types (id, label, base_url, models, driver_type, enabled, sort_order, docs_url) VALUES ($1, $2, $3, $4, $5, $6, $7, $8)"
                )
                .bind(&pt.id).bind(&pt.label).bind(&pt.base_url)
                .bind(&models_json).bind(&pt.driver_type).bind(pt.enabled.unwrap_or(true)).bind(pt.sort_order.unwrap_or(0)).bind(pt.docs_url.unwrap_or_default())
                .execute(pool).await?;
    ...
    pub async fn batch_insert_provider_types(&self, types: Vec<NewProviderType>) -> Result<()> {
        for (i, pt) in types.into_iter().enumerate() {
            ...
            match self {
                Self::Sqlite(pool) => {
                    sqlx::query(
                        "INSERT OR IGNORE INTO provider_types (id, label, base_url, models, driver_type, enabled, sort_order, docs_url) VALUES (?, ?, ?, ?, ?, ?, ?, ?)"
                    )
                    .bind(&pt.id).bind(&pt.label).bind(&pt.base_url)
                ...
                Self::Postgres(pool) => {
                    sqlx::query(
                        "INSERT INTO provider_types (id, label, base_url, models, driver_type, enabled, sort_order, docs_url) VALUES ($1, $2, $3, $4, $5, $6, $7, $8) ON CONFLICT (id) DO NOTHING"
                    )
                    .bind(&pt.id).bind(&pt.label).bind(&pt.base_url)
                    .bind(&models_json).bind(&pt.driver_type).bind(pt.enabled.unwrap_or(true)).bind(pt.sort_order.unwrap_or(i as i32)).bind(pt.docs_url.unwrap_or_default())
                    .execute(pool).await?;
