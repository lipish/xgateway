...
impl DatabasePool {
    ...
    pub async fn get_user_by_username(&self, username: &str) -> Result<Option<User>> {
        let query = "SELECT id, username, password_hash, role_id, status, created_at, updated_at FROM users WHERE username = ?";
        let pg_query = "SELECT id, username, password_hash, role_id, status, created_at, updated_at FROM users WHERE username = $1";
        ...
        match self {
            ...
            Self::Postgres(pool) => {
                Ok(sqlx::query_as::<_, User>(pg_query)
                    .bind(username)
                    .fetch_optional(pool)
                    .await?)
            }
    ...
    pub async fn list_users(&self) -> Result<Vec<User>> {
        ...
        match self {
            Self::Sqlite(pool) => Ok(sqlx::query_as::<_, User>(query).fetch_all(pool).await?),
